...
<head>
  <meta charset="UTF-8">
  <title>Kelola Peminjaman - Perpustakaan Admin</title>
    ...
    <tbody>
      ...
      ?>
        <tr>
          ...
          <td>
            <?php if (!$row['returned']): ?>
              <a href="borrows.php?return_id=<?= $row['id']; ?>" class="btn btn-sm btn-success mb-1" onclick="return confirm('Tandai buku ini sudah dikembalikan?')">Tandai Kembali</a>
            <?php endif; ?>
            <button class="btn btn-sm btn-secondary" data-bs-toggle="modal" data-bs-target="#fineModal<?= $row['id']; ?>">Atur Denda</button>
          </td>
        </tr>
            ...
            <div class="modal-content">
              <form method="POST">
                <div class="modal-header">
                  <h5 class="modal-title">Atur Denda untuk <?= htmlspecialchars($row['borrower_name']); ?></h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <input type="hidden" name="borrow_id" value="<?= $row['id']; ?>">
                  <div class="mb-3">
                    <label for="fine" class="form-label">Jumlah Denda (Rp)</label>
                    <input type="number" name="fine" class="form-control" value="<?= $row['fine']; ?>" required min="0">
                  </div>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                  <button type="submit" name="set_fine" class="btn btn-primary">Simpan Denda</button>
                </div>
              </form>
            </div>
          </div>
...
async function markReturned(borrowId) {
  ...
  try {
    const res = await fetch(`../api.php?resource=borrows&id=${borrowId}`, {
      method: 'PUT',
      credentials: 'include', // kirim session admin
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'return' })
    });
    const data = await res.json();
    if (res.ok && data.success) {
      alert('Berhasil dikembalikan. Denda: Rp ' + (data.fine || 0));
      location.reload();
    } else {
...
<!-- contoh tombol panggil fungsi (ganti link return_id menjadi tombol ini) -->
